...
def test_get_by_id(client):
    ...
    get_by_id = client.inventory.get_by_id(inv_id)
...
def test_inventory_crud(client):
    ...
    created = client.inventory.create(new_inv)
    this_id = created.id
    assert this_id is not None
    assert created.unit_category == UnitCategory.MASS.value
    deleted = client.inventory.delete(this_id)
    assert deleted
