...
def _split_patch_types_for_params_and_data_cols(
    *,
...
) -> tuple[list[GeneralPatchDatum], dict[str, list[dict]], list[dict]]:
    ...
    updated_sequences = [
        getattr(x, attribute_name) for x in updated_items if getattr(x, attribute_name) is not None
    ]

    to_add = [
        x
        for x in updated_items
        if getattr(x, attribute_name) not in existing_sequences
        and getattr(x, attribute_name) is not None
    ]
...
def generate_datatemplate_parameter_patches(
    *,
...
) -> tuple[list[GeneralPatchDatum], dict[str, list[dict]]]:
    """Generate patches for data template parameters.

    Parameters
    ----------
    existing : DataTemplate
        The existing data template.
    updated : DataTemplate
        The updated data template.

    Returns
    -------
    tuple[list[GeneralPatchDatum], dict[str, list[dict]]]
        The patches for the data template parameters and enum patches.
    """
    patches = []
    enum_patches = {}

    def handle_validation_updates(existing_col, updated_col) -> list[GeneralPatchDatum] | None:
        """Handle validation updates for a column."""
        if existing_col.validation != updated_col.validation:
            # Handle enum validations

            if (
                updated_col.validation
                and len(updated_col.validation) > 0
                and isinstance(updated_col.validation[0].value, list)
            ):
                existing_enums = (
                    []
                    if existing_col.validation is None
                    or len(existing_col.validation) == 0
                    or not isinstance(existing_col.validation[0].value, list)
                    else existing_col.validation[0].value
                )
                updated_enums = (
                    [] if updated_col.validation is None else updated_col.validation[0].value
                )

                existing_enum_ids = [
                    x.id for x in existing_enums if isinstance(x, EnumValidationValue)
                ]
                updated_enum_ids = [
                    x.id for x in updated_enums if isinstance(x, EnumValidationValue)
                ]
                enums_ids_to_update = set(existing_enum_ids) & set(updated_enum_ids)

                # Add new enum values
                new_enum_names = [x.text for x in updated_enums if x.id is None]
                deleted_enum_ids = set(existing_enum_ids) - set(updated_enum_ids)
                enum_patches[updated_col.sequence] = []
                for updated_enum in updated_enums:
                    if updated_enum.id in enums_ids_to_update:
                        this_existing_enum = [
                            x for x in existing_enums if x.id == updated_enum.id
                        ][0]
                        if this_existing_enum.text != updated_enum.text:
                            enum_patches[updated_col.sequence].append(
                                {
                                    "operation": "update",
                                    "id": updated_enum.id,
                                    "text": updated_enum.text,
                                }
                            )
                for new_enum_name in new_enum_names:
                    if updated_col.sequence not in enum_patches:
                        enum_patches[updated_col.sequence] = []
                    enum_patches[updated_col.sequence].append(
                        {
                            "operation": "add",
                ...
                for deleted_enum_id in deleted_enum_ids:
                    if updated_col.sequence not in enum_patches:
                        enum_patches[updated_col.sequence] = []
                    enum_patches[updated_col.sequence].append(
                        {
            ...
            # # Handle non-enum validation changes
            # validation_dump = []
            # if updated_col.validation:
            #     validation_dump = [
            #         x.model_dump(mode="json", by_alias=True, exclude_none=True)
            #         for x in updated_col.validation
            #     ]
            #     # If this is an enum validation, include the enum values
            #     if len(validation_dump) > 0 and isinstance(updated_col.validation[0].value, list):
            #         validation_dump[0]["value"] = [
            #             x.model_dump(mode="json", by_alias=True, exclude_none=True)
            #             for x in updated_col.validation[0].value
            #         ]

            # return GeneralPatchDatum(
            #     operation="add"
            #     if not existing_col.validation or len(existing_col.validation) == 0
            #     else "update",
            #     attribute="validation",
            #     rowId=updated_col.sequence,
            #     newValue=validation_dump,
            # )
            # Handle non-enum validation changes
            def _remove_enum_values_from_validation(validation):
                """Remove enum values from validation."""
            ...
            if updated_col.validation:
                validation_dump = [
                    _remove_enum_values_from_validation(x).model_dump(
                ...
                #         for x in updated_col.validation[0].value
                #     ]
            # existing_validation_dump = (
            #     None
            #     if existing_col.validation is None or len(existing_col.validation) == 0
            #     else [
            #         x.model_dump(mode="json", by_alias=True, exclude_none=True)
            #         for x in existing_col.validation
            #     ]
            # )
            patches = [
                GeneralPatchDatum(
    ...
    def handle_unit_updates(existing_col, updated_col):
        """Handle unit updates for a column."""
        if existing_col.unit != updated_col.unit:
            if existing_col.unit is None:
                return GeneralPatchDatum(
                    operation="add",
                    attribute="unitId",
                    rowId=updated_col.sequence,
                    newValue=updated_col.unit.id,
                )
            elif updated_col.unit is None:
                return GeneralPatchDatum(
                    operation="delete",
                    attribute="unitId",
                    rowId=updated_col.sequence,
                    oldValue=existing_col.unit.id,
                )
            else:
                if existing_col.unit.id == updated_col.unit.id:
                    return None
                return GeneralPatchDatum(
                    operation="update",
                    attribute="unitId",
    ...
    def handle_value_updates(existing_col, updated_col):
        """Handle value updates for a column."""
        if existing_col.value != updated_col.value:
            if existing_col.value is None:
                return GeneralPatchDatum(
                    operation="add",
                    attribute="value",
                    rowId=updated_col.sequence,
                    newValue=updated_col.value,
                )
            elif updated_col.value is None:
                return GeneralPatchDatum(
                    operation="delete",
                    attribute="value",
                    rowId=updated_col.sequence,
                    oldValue=existing_col.value,
                )
            else:
                return GeneralPatchDatum(
                    operation="update",
    ...
    # Process each column
    # either may be None, so we need to handle that
    existing_cols = existing.parameter_values if existing.parameter_values else []
    updated_cols = updated.parameter_values if updated.parameter_values else []
    for existing_col, updated_col in zip(existing_cols, updated_cols, strict=False):
        # Handle validation updates
        if patch := handle_validation_updates(existing_col, updated_col):
            patches.extend(patch)

        # Handle unit updates
        if patch := handle_unit_updates(existing_col, updated_col):
            patches.append(patch)

        # Handle value updates
        if patch := handle_value_updates(existing_col, updated_col):
            patches.append(patch)
